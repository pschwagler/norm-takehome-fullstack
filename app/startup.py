...
def startup(qdrant_service: QdrantService) -> None:
    ...
    with Session(engine) as session:
        doc_count = session.exec(select(LegislationDocument)).first()
        if doc_count is None:
            _seed_initial_document(session)

    # 5. Re-index all laws from DB into Qdrant
    _reindex_all(qdrant_service)


def _seed_initial_document(session: Session) -> None:
    ...
    logger.info("Seeding initial document from %s", SEED_PDF_PATH)

    doc_service = DocumentService()
    parsed_laws = doc_service.create_documents(SEED_PDF_PATH)
    ...
    session.commit()
    logger.info(
        "Seeded document id=%d with %d laws",
        document.id,
        len(parsed_laws),
    )
...
def _reindex_all(qdrant_service: QdrantService) -> None:
    ...
    with Session(engine) as session:
        ...
        for doc in documents:
            laws = list(
                session.exec(
                    select(Law).where(Law.document_id == doc.id)
                ).all()
            )
            ...
            leaf_nodes, nodes = doc_service.create_nodes(
                parsed_laws, doc.id, doc.name, doc.jurisdiction
            )
